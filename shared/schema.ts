...
export const serviceProviders = pgTable("service_providers", {
	...
	verificationSteps: jsonb("verification_steps").default(
		JSON.stringify({
			emailVerified: false,
...
export const insertClientApplicationSchema = createInsertSchema(applications)
	.omit({
		id: true,
		status: true,
		createdAt: true,
	...
	.extend({
		...
		confirmPassword: z
			.string()
			.min(6, "Konfirmasi password minimal 6 karakter"),
		intent: z.enum(["individu", "travel_agency"]).default("individu"),
		yearsInChina: z.number().optional(),
		googleId: z.string().optional(),
		studentEmail: z.string().optional(),
		questionnaireData: z.any().optional(), // Allow any questionnaire structure
...
export type ServiceProvider =
	| typeof serviceProviders.$inferSelect
