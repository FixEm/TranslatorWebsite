...
export default function SearchPage() {
  ...
  const { data: providers, isLoading } = useQuery<ServiceProvider[]>({
    ...
    queryFn: async () => {
      ...
      if (filters.service && filters.service !== "all")
        params.append("services", filters.service);
      ...
      const transformedStudents = students.map((student: any) => {
        ...
        if (
          ...
          student.questionnaireData.fluentLanguages &&
          Array.isArray(student.questionnaireData.fluentLanguages)
        ) {
          // Transform fluentLanguages array to the expected format
          languages = student.questionnaireData.fluentLanguages.map(
            (lang: string) => ({
              language: lang,
        ...
        allProviders = allProviders.filter((provider) => {
          ...
          return (
            ...
            provider.services.some(
              (service: string) => service === filters.service
            )
          );
        });
      }
      ...
      if (filters.availability) {
        const requestedDate = filters.availability;
        const requestedStr = `${requestedDate.getFullYear()}-${String(
          requestedDate.getMonth() + 1
        ).padStart(2, "0")}-${String(requestedDate.getDate()).padStart(
          2,
          "0"
        )}`;
          ...
          allProviders.map(async (provider: any) => {
            ...
            try {
              const resp = await fetch(
                `/api/provider-calendars/${provider.id}`
              );
              if (resp.ok) {
                const calendar = await resp.json();
                const unavailable: string[] = Array.isArray(
            ...
            if (!availability) {
              try {
                if ((provider as any).isStudent) {
                  const a = await fetch(
                    `/api/applications/${provider.id}/availability`
                  );
  ...
  const handleTodayAvailability = () => {
    setFilters((prev) => ({ ...prev, availability: new Date() }));
    setCalendarOpen(false);
  };
  ...
  const handleAvailabilityChange = (date: Date | undefined) => {
    setFilters((prev) => ({ ...prev, availability: date || null }));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">
            Gunakan filter canggih untuk menemukan profesional terverifikasi
        ...
        <Card className="mb-12 shadow-lg border-0 rounded-2xl bg-white">
          <CardContent className="p-8">
            <div className="flex items-center mb-8">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  Temukan translator yang tepat untuk kebutuhan Anda
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              ...
              <div className="space-y-3">
                <label className="rounded-xl text-sm font-semibold text-gray-700 flex items-center">
                ...
                <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      className={cn(
                        "w-full justify-start text-left font-normal border-gray-200 focus:border-red-500 focus:ring-red-500 rounded-xl h-14 bg-gray-50 hover:bg-white hover:text-gray-900 transition-colors",
                        !filters.availability && "text-muted-foreground"
                      )}
                    >
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {filters.availability
                        ? formatAvailabilityDate(filters.availability)
                        : "mm/dd/yyyy"}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar
                      mode="single"
                      selected={filters.availability || undefined}
            ...
            {providers.length === 0 && (
              <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-gray-100">
                ...
                <h3 className="text-2xl font-bold text-gray-900 mb-4">
                  Tidak ada translator ditemukan
                </h3>
                <p className="text-gray-600 text-lg mb-8 max-w-md mx-auto">
                  Coba ubah kriteria pencarian atau filter untuk hasil yang
                  lebih luas
                </p>
                <Button
                  onClick={() =>
