...
export const emailPatterns = {
	...
	indonesianUniversities:
		/@(ui\.ac\.id|ugm\.ac\.id|itb\.ac\.id|unair\.ac\.id|undip\.ac\.id|student\.ac\.id)$/,
	chineseUniversities:
		/@(tsinghua\.edu\.cn|pku\.edu\.cn|fudan\.edu\.cn|sjtu\.edu\.cn|zju\.edu\.cn|nju\.edu\.cn|ustc\.edu\.cn|hit\.edu\.cn|xjtu\.edu\.cn|nwpu\.edu\.cn|buaa\.edu\.cn|bit\.edu\.cn|bnu\.edu\.cn|ruc\.edu\.cn|cuhk\.edu\.cn|hku\.edu\.cn)$/,
};
...
export function isIndonesianStudentEmail(email: string): boolean {
	return (
		emailPatterns.studentAcId.test(email) ||
...
export function isChineseUniversityEmail(email: string): boolean {
	return (
		emailPatterns.eduCn.test(email) ||
...
export async function createUserAndSendVerification(
	...
	try {
		...
		if (!isStudentEmail(email)) {
			throw new Error(
				"Silakan gunakan email mahasiswa Anda (@student.ac.id atau @edu.cn) untuk verifikasi."
			);
		}
...
// Create Firebase user without student email check (bypass verification)
export async function createUserWithoutStudentEmailCheck(
	...
	try {
		...
		// Send verification email using the new client email service
