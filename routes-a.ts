...
export async function registerRoutes(app: Express): Promise<Server> {
	...
	app.post("/api/applications/submit", async (req, res) => {
		try {
			...
			try {
				...
				console.log(
					"❌ [JSON APPLICATION] Raw validation failed:",
					validationError
				);
				if (
					validationError &&
					...
					console.log(
						"🔍 [JSON APPLICATION] Validation issues:",
			...
			try {
				...
				res.status(201).json({
					...
					message:
						"Akun berhasil dibuat tetapi gagal mengirim email verifikasi",
				});
			}
		} catch (error) {
			...
			try {
				...
				if (req.body.services) {
					try {
						...
						console.warn(
							"⚠️ [TRANSLATOR] Failed to parse services, using empty array:",
							req.body.services
						);
						services = [];
					}
				...
				if (req.body.password && req.body.email) {
					try {
						...
						console.log(
							"✅ [TRANSLATOR] Firebase user created:",
							uid,
							"for",
							req.body.email
						);
						...
						if (emailSent) {
							console.log(
								"📧 [TRANSLATOR] Email verifikasi terkirim ke",
								req.body.email
							);
						} else {
							console.log(
								"📧 [TRANSLATOR] Email sudah diverifikasi sebelumnya"
							);
						}
					} catch (firebaseError: any) {
						console.error(
							"❌ [TRANSLATOR] Firebase user creation failed:",
						...
						if (firebaseError.code === "auth/email-already-in-use") {
							console.log(
								"📧 [TRANSLATOR] Email already exists in Firebase, linking to existing user"
							);
							try {
								...
								console.error(
									"❌ [TRANSLATOR] Failed to link existing user:",
									linkError
								);
							}
						}
				...
				res.status(201).json({
					...
					application,
					message:
						"Aplikasi penerjemah berhasil dikirim! Silakan periksa email untuk verifikasi.",
				});
			} catch (error) {
	...
	app.get("/api/applications/verified", async (req, res) => {
		try {
			...
			console.log(
				`🔥 Found ${verifiedUsers.length} verified users from ${allApplications.length} total applications`
			);
	...
	app.patch("/api/applications/:id/status", async (req, res) => {
		try {
			...
			if (
				!["approved", "rejected", "pending", "needs_changes"].includes(status)
			) {
			...
			if (status === "approved") {
				...
				if (existingProviders.length === 0) {
					const providerData = {
						...
						certificates: Array.isArray(application.certificates)
							? application.certificates
							: [],
						introVideo: application.introVideo, // Include intro video URL
						intent: (application.intent || "translator") as
							| "translator"
							| "tour_guide"
							| "both",
					};
	...
	app.patch("/api/applications/:id/recruitment-status", async (req, res) => {
		try {
			console.log("🎯 UPDATE RECRUITMENT STATUS:", {
				id: req.params.id,
				status: req.body.recruitmentStatus,
			});
			const { id } = req.params;
			...
			if (recruitmentStatus === "approved") {
				const currentApp = await storage.getApplication(id);
				const currentVerificationSteps =
					(currentApp as any)?.verificationSteps || {};
				...
				console.log(
					"✅ Set adminApproved to true and finalStatus to approved for candidate"
				);
			...
			console.log(
				"✅ Recruitment status updated for:",
				updatedApplication?.email
			);
			res.json({
	...
	app.patch("/api/applications/:id/revoke-verification", async (req, res) => {
		try {
			...
			console.log(
				"🔄 Revoking verification and clearing accept/reject status..."
			);
			...
			await storage.updateApplicationField(
				...
				"recruitmentStatus",
				"pending_interview"
			); // Reset to initial recruitment stage
	...
	app.patch("/api/applications/:id/request-changes", async (req, res) => {
		try {
			...
			try {
				...
				console.error(
					"⚠️ Error deleting files (continuing anyway):",
					deleteError
				);
	...
	app.put("/api/applications/:id/hsk-level", async (req, res) => {
		try {
			console.log("📚 Updating HSK level:", {
				id: req.params.id,
		...
		try {
			...
			if (userApplication && userApplication.password) {
				...
				try {
					...
					console.log(
						"Could not get Firebase user verification status:",
						error
					);
					emailVerified = false;
				}
				...
				console.log(
					`✅ Application login successful for: ${email}, role: ${userRole}, emailVerified: ${emailVerified}`
				);
			...
			try {
				...
				if (userRecord) {
					...
					console.log(
						"Firebase Auth user found, but password verification not implemented"
					);
				}
			} catch (firebaseError) {
	...
	app.patch(
		"/api/applications/:id/documents/:documentType/status",
		async (req, res) => {
			try {
				...
				if (
					!["pending", "approved", "rejected", "needs_changes"].includes(status)
				) {
					return res.status(400).json({ message: "Invalid status" });
				}
				...
				if (application) {
					const verificationSteps =
						(application.verificationSteps as any) || {};
					...
					if (
						...
						verificationSteps.studentIdStatus === "approved"
					)
						completedSteps++;
					if (
						verificationSteps.hskUploaded &&
						verificationSteps.hskStatus === "approved"
					)
						completedSteps++;
					if (
						verificationSteps.cvUploaded &&
						verificationSteps.cvStatus === "approved"
					)
						completedSteps++;
	...
	// Client application submission (no file upload)
	app.post("/api/applications/client", async (req, res) => {
		try {
			console.log(
				"📥 [CLIENT APPLICATION] Request body received (before flattening):",
				{
					...req.body,
					password: req.body.password ? "[HIDDEN]" : "NO PASSWORD",
				}
			...
			// Flatten the nested questionnaireData object into the main body
			...
			console.log(
				"📥 [CLIENT APPLICATION] Request body received (after flattening):",
				{
					...req.body,
					password: req.body.password ? "[HIDDEN]" : "NO PASSWORD",
				}
			...
			// Validate the raw data using the new client application schema
			console.log(
				"🔍 [CLIENT APPLICATION] Validating raw data using schema:",
				insertClientApplicationSchema
			);

			try {
				const rawValidation = insertClientApplicationSchema.parse(req.body);
				console.log("✅ [CLIENT APPLICATION] Raw validation successful");
			} catch (validationError) {
				console.log(
					"❌ [CLIENT APPLICATION] Raw validation failed:",
					validationError
				);
				if (
					validationError &&
					...
					const issues = (validationError as any).issues;
					console.log(
						"🔍 [CLIENT APPLICATION] Validation issues:",
						JSON.stringify(issues, null, 2)
					);
					return res.status(400).json({
						message: "Validation error",
			...
			let hashedPassword = undefined;
			if (req.body.password) {
				console.log("🔒 [CLIENT APPLICATION] Hashing password...");
				hashedPassword = await bcrypt.hash(req.body.password, 10);
				console.log("🔒 [CLIENT APPLICATION] Password hashed successfully");
			}
			...
			console.log("📝 [CLIENT APPLICATION] Application data being saved:", {
				...applicationData,
				password: applicationData.password ? "[HASHED]" : "NO PASSWORD",
				intent: applicationData.intent,
			});
			...
			// Log the application ID after saving
			console.log(
				`✅ [CLIENT APPLICATION] Application saved with ID: ${application.id}`
			);
			...
			try {
				...
				console.log(
					"📧 [CLIENT APPLICATION] Creating Firebase user and sending verification email..."
				);
				...
				console.log(
					`📧 [CLIENT APPLICATION] Email verification ${
						emailSent ? "sent" : "attempted"
				...
				console.error(
					"❌ [CLIENT APPLICATION] Failed to create Firebase user or send verification email:",
					userError
				);
				res.status(201).json({
					...application,
